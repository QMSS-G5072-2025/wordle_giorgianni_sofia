def validate_guess(guess, word_length=5):
    ...
    >>> validate_guess("cat")
    False
    >>> validate_guess("notfive")
    False

    Only letters allowed:
    >>> validate_guess("cr4ne")
    False
    >>> validate_guess("cr ne")
    False
...
def check_guess(secret_word, guess):
    ...
    No matches (all gray):
    >>> check_guess("crane", "blimp")
    [('b', 'gray'), ('l', 'gray'), ('i', 'gray'), ('m', 'gray'), ('p', 'gray')]

    Mixed (greens, yellows, gray):
    >>> check_guess("react", "crane")  # doctest: +NORMALIZE_WHITESPACE
    [('c', 'yellow'), ('r', 'yellow'), ('a', 'green'), ('n', 'gray'), ('e', 'yellow')]

    If lengths differ, return empty list:
    >>> check_guess("crane", "toolong")
    []
    """
